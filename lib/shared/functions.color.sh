...
{
	athena.color.print_color "blue" "[INFO]" " $1"
}
...
{
	athena.color.print_color "red" "[ERROR]" " $1"
}
...
{
	athena.color.print_color "green" "[OK]" " $1"
}
...
{
	athena.color.print_color "yellow" "[WARN]" " $1"
}
...
function athena.color.print_debug()
{
	if athena.os.is_debug_active ; then
		athena.color.print_color "cyan" "[DEBUG]" " $1"
	fi
}
...
{
	exit_code=${2:-1}
	athena.color.print_color "red" "[FATAL]" " $1"
